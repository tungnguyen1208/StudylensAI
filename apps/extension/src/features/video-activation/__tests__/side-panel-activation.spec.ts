...
describe('Side Panel activation state', () => {
  it('maps enabled and disabled envelopes without accessing YouTube DOM', () => {
    const active = applyVideoActivationMessage(initialActivationState, {
      type: 'ACTIVATION_ENABLED', contractVersion: '0.4.0', correlationId: 'b', tabId: 7,
      youtubeVideoId: 'dQw4w9WgXcQ', occurredAtUtc: '2026-09-13T10:00:01.000Z',
      payload: { activationId: 'a', source: 'user', videoTitle: 'Networking lesson', transcriptCapture: { transcriptCaptureId: 'capture-1', youtubeVideoId: 'dQw4w9WgXcQ', language: 'en', source: 'youtubeCaption', status: 'available', availableCueCount: 1, version: 1 }, preferences: { quizIntervalMinutes: 10, questionType: 'multipleChoice', difficulty: 'medium' } },
    });
    const stopped = applyVideoActivationMessage(active, {
      type: 'ACTIVATION_DISABLED', contractVersion: '0.4.0', correlationId: 'c', tabId: 7,
  ...
  it('keeps the global toggle on while the next page waits for transcript evidence', () => {
    const transitioned = applyVideoActivationMessage(initialActivationState, {
      type: 'VIDEO_CONTEXT_CHANGED', contractVersion: '0.4.0', correlationId: 'transition', tabId: 7,
  ...
  it('keeps the global toggle on when navigation leaves a supported watch page', () => {
    const waiting = applyVideoActivationMessage(initialActivationState, {
      type: 'VIDEO_CONTEXT_UNAVAILABLE', contractVersion: '0.4.0', correlationId: 'away', tabId: 7,

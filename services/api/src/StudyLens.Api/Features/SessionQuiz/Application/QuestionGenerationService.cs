...
public sealed class QuestionGenerationService
{
    ...
    public async Task<QuizGenerationResult> GenerateAsync(GenerateQuizCommand command, CancellationToken cancellationToken)
    {
        ...
        var generated = await _gateway.GenerateAsync(new("0.2.0", "0.2.0", command.SegmentId, command.YoutubeVideoId, startMs, endMs, command.QuestionType, command.Difficulty, cues), cancellationToken);
